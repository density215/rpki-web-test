...
const addConsoleLine = (line, textClass, overwrite = false) => {
  console.log(line);
  ...
  if (!overwrite) {
    ...
    consoleElm.lastChild.replaceWith(termLine);
  }
};
...
const finalMsg = rpkiResult => {
  ...
  if (!rpkiResult.asn) {
    ...
    addConsoleLine(
      `AS${(rpkiResult.asn.length > 1 && "s") || ""}${rpkiResult.asn.join(
        ","
      )} ${msgVerb}${(rpkiResult.asn.length === 1 && "s") ||
        ""} RPKI invalid BGP routes from prefix ${
        rpkiResult.pfx
      } as witnessed by your public IP ${rpkiResult.ip}`,
      textClass
    );
  }
};
